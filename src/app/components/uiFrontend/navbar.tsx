...
export function NavbarComp({ className }: { className?: string }) {
  ...
  return (
    <div
      className={cn("flex flex-col fixed top-10 inset-x-0 max-w-2xl mx-auto z-50 text-xl", className)}
    >
      <Menu setActive={setActive}>
        <MenuItem setActive={setActive} active={active} item="Work">
          <div className="flex flex-col space-y-4 text-lg">
            <HoveredLink href="/orocorp">Orocorp</HoveredLink>
            <HoveredLink href="/honest-bites">Honest Bites</HoveredLink>
            <HoveredLink href="/seo">Search Engine Optimization</HoveredLink>
            <HoveredLink href="/branding">Branding</HoveredLink>
          </div>
        </MenuItem>
        <MenuItem setActive={setActive} active={active} item="Cool Stuff">
          <div className="text-sm grid grid-cols-2 gap-y-12 p-4">
            <ProductItem
              title="Algochurn"
